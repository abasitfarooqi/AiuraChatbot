...
start_server() {
    ...
    nohup uvicorn backend.app.main:app --host 0.0.0.0 --port 8000 > logs/server.log 2>&1 &
    SERVER_PID=$!
    ...
    if ps -p $SERVER_PID > /dev/null 2>&1; then
        ...
        echo -e "${YELLOW}📝 Logs: tail -f logs/server.log${NC}"
        return 0
    else
...
start_tunnel() {
    ...
    if ps -p $TUNNEL_PID > /dev/null 2>&1; then
        if [ -n "$TUNNEL_URL" ]; then
            ...
            echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
            echo ""
            echo -e "${GREEN}Public URL:${NC} ${YELLOW}$TUNNEL_URL${NC}"
            echo -e "${GREEN}API Base:${NC}   ${YELLOW}$TUNNEL_URL/api/v1${NC}"
            echo -e "${GREEN}API Docs:${NC}   ${YELLOW}$TUNNEL_URL/docs${NC}"
            echo -e "${GREEN}Health:${NC}     ${YELLOW}$TUNNEL_URL/health${NC}"
            echo ""
...
status() {
    ...
    if check_tunnel_running; then
        ...
        if [ -n "$TUNNEL_URL" ]; then
            ...
            echo -e "   ${BLUE}API Base:${NC}   $TUNNEL_URL/api/v1"
            echo -e "   ${BLUE}API Docs:${NC}   $TUNNEL_URL/docs"
            echo -e "   ${BLUE}Health:${NC}     $TUNNEL_URL/health"
            echo ""
            echo -e "   ${YELLOW}Laravel .env:${NC}"
            echo -e "   ${YELLOW}CHATBOT_API_BASE=$TUNNEL_URL/api/v1${NC}"
        else
...
case "${1:-}" in
    ...
    *)
        print_header
        echo "Usage: $0 {start|stop|restart|expose|tunnel|status}"
        echo ""
        echo "Commands:"
        ...
        echo "  status    - Show server and tunnel status"
        echo ""
        echo "Examples:"
