...
Uses root password abc123 (as configured in database.py).
"""
import os
import sys
import pymysql
...
def list_tables():
    ...
    try:
        ...
        print('1. Check if MySQL is running: brew services list | grep mysql')
        print('2. Verify password in backend/config/database.py')
        print('3. Try: mysql -u root -pabc123 aiura_chatbots -e "SHOW TABLES;"')
        sys.exit(1)
    except Exception as e:
        print(f'❌ Error: {e}')
        import traceback
